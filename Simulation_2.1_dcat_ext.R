...
for (s in 1:num_datasets){
  ...
  sim_model_b <- nimbleModel(code_sim_b[[s]], 
                             ...
                             inits = list(phiB = 0.6, phiNB = 0.5,
                                          pB=0.7, pNB=0.4,
                                          psiB_NB=0.3, psiNB_B=0.4,
...
for (s in 1:num_datasets){
  ...
  s_pB[s] <- MCMCsummary(samples[[s]], params = "pB", round=5)$mean
  s_pNB[s] <- MCMCsummary(samples[[s]], params = "pNB", round=5)$mean
  s_phiB[s] <- MCMCsummary(samples[[s]], params = "phiB", round=5)$mean
  s_phiNB[s] <- MCMCsummary(samples[[s]], params = "phiNB", round=5)$mean
  s_psiB_NB[s] <- MCMCsummary(samples[[s]], params = "psiB_NB", round=5)$mean
  s_psiNB_B[s] <- MCMCsummary(samples[[s]], params = "psiNB_B", round=5)$mean
