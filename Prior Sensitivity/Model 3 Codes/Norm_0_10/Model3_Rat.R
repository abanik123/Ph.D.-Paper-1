...
#' Read in the data. 
## ---------------------------------------------------------------------------------------------------------
data <- read.csv("Rat_Breeding_Sex_Data.csv")
y <- data %>%
...
pdf(file = "Rat_mp_m3.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Rat_m3", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Rat_m3_sum.csv")
