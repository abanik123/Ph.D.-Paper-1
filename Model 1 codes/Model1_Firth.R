...
capture_mat <- read.csv("Firth_Capture_Data.csv")
...
pdf(file = "Firth_mp_m1.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Firth_m1", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Firth_m1_sum.csv")