...
capture_mat <- read.csv("Big_Capture_Data.csv")
...
# GeneBige inits for the latent states
...
pdf(file = "Big_mp_m1.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_m1", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Big_m1_sum.csv")