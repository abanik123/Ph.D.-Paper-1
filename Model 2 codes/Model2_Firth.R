...
#' Read in the data. 
## ---------------------------------------------------------------------------------------------------------
data <- read.csv("Firth_Breeding_Data.csv")
y <- data %>%
...
pdf(file = "Firth_mp_m2.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Firth_m2", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Firth_m2_sum.csv")