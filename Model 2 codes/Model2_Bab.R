...
#' Read in the data. 
## ---------------------------------------------------------------------------------------------------------
data <- read.csv("Babbage_Breeding_Data.csv")
y <- data %>%
...
pdf(file = "Babbage_mp_m2.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Babbage_m2", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Babbage_m2_sum.csv")