...
code_m <- nimbleCode({
  ...
  for (i in 1:N) {
    ...
    logit(phiB[i])<- (a_f[1]*f[i] + a_bab[1]*bab[i] +
                                a_big[1]*big[i] + a_r[1]*r[i] + a_j[1]*j[i])
    
    logit(psiB_NB[i]) <- (a_f[2]*f[i] + a_bab[2]*bab[i] +
                                    a_big[2]*big[i] + a_r[2]*r[i] + a_j[2]*j[i])
    
    logit(phiNB[i])<- (a_f[3]*f[i] + a_bab[3]*bab[i] +
                                 a_big[3]*big[i] + a_r[3]*r[i] + a_j[3]*j[i])
    
    logit(psiNB_B[i])<- (a_f[4]*f[i] + a_bab[4]*bab[i] +
...
pdf(file = "All_mp_m2.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "All_m2", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "All_m2_sum.csv")
